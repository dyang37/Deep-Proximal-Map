import copy
import sys
sys.path.append('/Users/dyangsteven/Plug_n_play/mypnp/python/util/')
sys.path.append('/Users/dyangsteven/Plug_n_play/mypnp/python/denoisers/DnCNN/')
import dncnn
import matplotlib
matplotlib.use("TkAgg")
import matplotlib.pyplot as plt 
from dncnn import cnn_denoiser
from admm_util import shepard_init
from skimage.io import imread
import numpy as np
from math import sqrt
from skimage.restoration import denoise_tv_chambolle as denoiser_tv
from skimage.measure import compare_psnr
from PIL import Image
import ADMM as admm

denoiser=int(sys.argv[1])
if denoiser == 0:
  print('using neural network denoiser...')
else:
  print('using total variation denoiser...')
noise_level = 10./255.;
rho = 1.
gamma = 1.
max_itr = 50
tol = 10**-4
lambd = 1./20.
# data pre-processing: apply mask and add AWGN
z = np.array(imread('./data/Art512.png'), dtype=np.float32) / 255.0
print('input image size: ',np.shape(z))
[row,col]=np.shape(z)
N=row*col
np.random.seed(0)
mask = np.random.rand(row,col)
mask=mask>=0.8
gauss = np.random.normal(0,1,(row,col))
z_mask=z*mask
y=np.clip(z_mask+noise_level*gauss,0,1)

# ADMM initialization
Sty = y*mask

v = np.random.rand(row,col)

x = v
#u = np.zeros([row,col])
u = shepard_init(y, mask, 10) 
residual = float("inf")
itr = 0

Amatrix = np.transpose(admm.generateAmatrix(mask, 0))
# ADMM recursive update
while (residual > tol) and (itr < max_itr):
  err = (1/sqrt(N))*(sqrt(sum(sum((x-z)**2))))
  x_old = copy.copy(x)
  v_old = v
  u_old = u
  # inversion
  xtilde = v-u
  rhs = Sty + rho*xtilde
  x = rhs/(mask+rho) 
  if itr == 0:
    icd_niter = 10
  else:
    icd_niter = 1
  x = admm.DataTermOptInPaint(x, y, icd_niter, u, v, lambd, Amatrix)
  # denoising
  vtilde=x+u
  vtilde = vtilde.clip(min=0,max=1)
  sigma = sqrt(lambd/rho)
  if denoiser == 0:
    v = cnn_denoiser(vtilde)
  else:
    v = denoiser_tv(vtilde)
  # update u
  u = u+(x-v)
  # update rho
  rho=rho*gamma
  residualx = (1/sqrt(N))*(sqrt(sum(sum((x-x_old)**2))))
  residualv = (1/sqrt(N))*(sqrt(sum(sum((v-v_old)**2))))
  residualu = (1/sqrt(N))*(sqrt(sum(sum((u-u_old)**2))))
  residual = residualx + residualv + residualu
  itr = itr + 1
  #print(itr,' ',residualx,' ', residualv,'  ', residualu)
  print(itr,' ',err)
# end of ADMM recursive update
psnr = compare_psnr(z, x)
print('PSNR of restored image: ',psnr)

plt.figure()
plt.subplot(131)
plt.imshow(z,interpolation='nearest',cmap='gray')
plt.title('original image')
plt.subplot(132)
plt.imshow(y,interpolation='nearest',cmap='gray')
plt.title('noisy image')
plt.subplot(133)
plt.imshow(x,interpolation='nearest',cmap='gray')
plt.title('reconstructed image')
plt.show()

