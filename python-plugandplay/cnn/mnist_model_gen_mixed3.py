...
model_name = "mnist_forward_newModel"
_train = False
_log_data = False
...
np.random.seed(2019)
for v_img in x_train[0:n_samples//3]:
...
for x_img in x_train[n_samples//3:2*n_samples//3]:
  sig = random.uniform(0,0.3)
...
x_idx_list = np.random.choice(range(n_samples*2//3,n_samples-100), size=n_samples//3-100, replace=False)
v_idx_list = np.random.choice(range(n_samples*2//3,n_samples-100), size=n_samples//3-100, replace=False)
...
yAv = np.reshape(yAv,(n_samples-100,10))
print("shape of v: ",np.shape(v))
print("shape of y-Av: ",np.shape(yAv))
if _log_data:
  dataset = {'epsil':epsil,'log_y_Av':yAv, 'v':v}
  dict_name = '/root/datasets/'+datagen_method+'/mnist_mixed3_log_triplets_sigvar_sigw'+str(sigw)+'.dat'
else:
  dataset = {'epsil':epsil,'y_Av':yAv, 'v':v}
  dict_name = '/root/datasets/'+datagen_method+'/mnist_mixed3_triplets_sigvar_sigw'+str(sigw)+'.dat'
